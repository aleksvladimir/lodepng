...
//  lodepng::load_file(in, infilename);
  ...
  size_t in_size;                               /* how much input did we read? */
  unsigned char in[100]; /* Example-only buffer, you'd replace it with other global or
  ...
  lodepng_deflate(&out, &outsize, &in[0], /*in.size()*/in_size, &lodepng_default_compress_settings);

  unsigned crc = lodepng_crc32(&in[0], /*in.size()*/in_size);
  ...
  //ISIZE
  out[footer + 4] = /*in.size()*/in_size % 256;
  out[footer + 5] = (/*in.size()*/in_size >> 8) % 256;
  out[footer + 6] = (/*in.size()*/in_size >> 16) % 256;
  out[footer + 7] = (/*in.size()*/in_size >> 24) % 256;
